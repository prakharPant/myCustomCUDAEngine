...
__global__ void level2_tile_attn_kernel(
    ...
    // 3. S = Q @ K^T (Use ldmatrix_x2_trans to get K^T)
    uint32_t K0_frag[2], K1_frag[2];
    {
        int k0_row = lane_id % 8;
        int k0_col = (lane_id / 8) * 8;
        ldmatrix_x2_trans(K0_frag, &smem_K[k0_row * 16 + k0_col]);

        int k1_row = 8 + (lane_id % 8);
        int k1_col = (lane_id / 8) * 8;
        ldmatrix_x2_trans(K1_frag, &smem_K[k1_row * 16 + k1_col]);
    }
    ...
    // 7. O = P @ V (Use NON-transposed ldmatrix_x2 for V)
    uint32_t P_frag[4];
    {
    ...
    uint32_t V0_frag[2], V1_frag[2];
    {
        ...
        int v1_col = 8;
        ldmatrix_x2(V0_frag, &smem_V[v0_row * 16 + v0_col]); // Fixed: ldmatrix_x2
        ldmatrix_x2(V1_frag, &smem_V[v1_row * 16 + v1_col]); // Fixed: ldmatrix_x2
    }
